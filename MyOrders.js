...
const MyOrders = () => {
    ...
    function getMyOrders() {
        axios
        .get(`http://localhost:8080/Orders/GetAllOrdersByUser/${loggeduser}`)
        .then((res) => {
    ...
    const updateOrder = (id, status) => {
        if (status === "placed" || status === "delivered") {
            ...
            axios.put(`http://localhost:8080/Orders/UpdateStatus/${id}`,data)
                .then((res) => {
    ...
    const rateOrder =(orderId, rating) => {
        ...
        axios.put(`http://localhost:8080/Orders/AddRating/${orderId}/${rating}`)
        .then((res) =>{
                        ...
                        {orders.map((order) => (
                            <Card key={order?.id} className="my-3 shadow">
                                <Card.Body>
                                    ...
                                    <tr key={item?.id} className="text-center text-capitalized">
                                    <td>{item.name}</td>
                                    <td>₹{item.price}</td>
                                    </tr>
                                      ))}
